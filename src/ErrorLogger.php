...
final class ErrorLogger
{
    ...
    public static function init(string $pluginDirectory, string $pluginFile): void
    {
        ...
        if (!$initialized) {
            self::$errorLogger = self::getLoggerInstance($pluginFile);
    ...
    public static function getLoggerInstance(string $pluginFile): Common\Backend\ErrorLogger
    {
        return Common\Backend\ErrorLogger::getInstance(
            Main::getShopDomain(),
            'WooCommerce',
            'plugins/' . plugin_basename($pluginFile),
