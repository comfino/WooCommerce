...
0!==b.loanTerm&&(Comfino.loanParams.loanAmount=b.loanAmount,Comfino.loanParams.loanType=b.loanType,Comfino.loanParams.loanTerm=b.loanTerm)};comfinoSettings.paywallOptions.onOfferLoadSuccess=b=>{ComfinoPaywallFrontend.logEvent("onOfferLoadSuccess WooCommerce (Payment Blocks)","debug",b);b=document.getElementById("comfino-paywall-container");if("block"===b.style.display&&!ComfinoPaywallFrontend.isLoaded())ComfinoPaywallFrontend.onload(b,comfinoSettings.paywallOptions.platformName,comfinoSettings.paywallOptions.platformVersion)};
e=document.getElementById("comfino-paywall-container");if(null===e){a=document.getElementById("comfino-gateway-logo");if(null===a){ComfinoPaywallFrontend.logEvent("Comfino logo not found in the payment block.","error");return}if(null===Comfino.listItemContainer){a=ComfinoPaywallFrontend.findMatchingParentElement(a,b=>null!==b.querySelector('input[type="radio"][value="comfino"]'));if(null===a){ComfinoPaywallFrontend.logEvent("Comfino initialization input element not found in the payment block.","error");
return}Comfino.listItemContainer=ComfinoPaywallFrontend.findMatchingParentElement(a,b=>"DIV"===b.tagName)}null===Comfino.labelObserver&&(Comfino.labelObserver=new MutationObserver((b,c)=>{ComfinoPaywallFrontend.logEvent("Comfino item changed.","debug",b,c);c=document.getElementById("comfino-paywall-container");if(null!==c&&"block"!==c.style.display)if(Comfino.isSelected)ComfinoPaywallFrontend.logEvent("Deferred Comfino initialization started.","debug",c),ComfinoPaywallFrontend.init(null,c,comfinoSettings.paywallOptions),
