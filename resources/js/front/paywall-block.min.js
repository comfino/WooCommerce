...
label},Content:a=>{const {eventRegistration:e,emitResponse:b}=a,{onPaymentSetup:c}=e;wp.element.useEffect(()=>{const d=c(async()=>({type:b.responseTypes.SUCCESS,meta:{paymentMethodData:{comfino_loan_amount:Comfino.loanParams.loanAmount.toString(),comfino_loan_type:Comfino.loanParams.loanType,comfino_loan_term:Comfino.loanParams.loanTerm.toString()}}}));return()=>{d()}},[b.responseTypes.SUCCESS,c]);null===Comfino.paywallTemplate&&(a=(new DOMParser).parseFromString(comfinoSettings.iframeTemplate,"text/html"),
...
a);if(Comfino.isPaywallActive)if("undefined"===typeof ComfinoPaywallFrontend)console.warn("ComfinoPaywallFrontend is undefined.");else{ComfinoPaywallFrontend.logEvent("Comfino.init - initialization started.","debug");if(ComfinoPaywallFrontend.isInitialized()){if(ComfinoPaywallFrontend.logEvent("Paywall frontend already initialized.","debug",Comfino.loanParams),0<Comfino.loanParams.loanAmount){var e=parseInt(a.cartTotals.total_price);Comfino.loanParams.loanAmount!==e&&(ComfinoPaywallFrontend.logEvent("Total value changed.",
"debug",Comfino.loanParams.loanAmount,e),ComfinoPaywallFrontend.reloadPaywall())}}else{ComfinoPaywallFrontend.logEvent("Paywall frontend initialization.","debug",comfinoSettings);comfinoSettings.paywallOptions.onUpdateOrderPaymentState=b=>{ComfinoPaywallFrontend.logEvent("updateOrderPaymentState WooCommerce (Payment Blocks)","debug",b);0!==b.loanTerm&&(Comfino.loanParams.loanAmount=b.loanAmount,Comfino.loanParams.loanType=b.loanType,Comfino.loanParams.loanTerm=b.loanTerm)};e=document.getElementById("comfino-paywall-container");
if(null===e){a=document.getElementById("comfino-gateway-logo");if(null===a){ComfinoPaywallFrontend.logEvent("Comfino logo not found in the payment block.","error");return}if(null===Comfino.listItemContainer){a=ComfinoPaywallFrontend.findMatchingParentElement(a,b=>null!==b.querySelector('input[type="radio"][value="comfino"]'));if(null===a){ComfinoPaywallFrontend.logEvent("Comfino initialization input element not found in the payment block.","error");return}Comfino.listItemContainer=ComfinoPaywallFrontend.findMatchingParentElement(a,
b=>"DIV"===b.tagName)}null===Comfino.labelObserver&&(Comfino.labelObserver=new MutationObserver((b,c)=>{ComfinoPaywallFrontend.logEvent("Comfino item changed.","debug",b,c);c=document.getElementById("comfino-paywall-container");if("block"!==c.style.display)if(Comfino.isSelected)ComfinoPaywallFrontend.logEvent("Deferred Comfino initialization started.","debug",c),ComfinoPaywallFrontend.init(null,c,comfinoSettings.paywallOptions),ComfinoPaywallFrontend.executeClickLogic();else for(let d=0;d<b.length;d++){let f;
f="INPUT"===b[d].target.tagName&&"radio"===b[d].target.type?b[d].target:b[d].target.querySelector('input[type="radio"]');if(null!==f&&"comfino"===f.value){Comfino.isSelected=!0;ComfinoPaywallFrontend.logEvent("Comfino item selected.","debug",f);ComfinoPaywallFrontend.logEvent("Deferred Comfino initialization started.","debug",c);ComfinoPaywallFrontend.init(null,c,comfinoSettings.paywallOptions);ComfinoPaywallFrontend.executeClickLogic();break}}}),Comfino.labelObserver.observe(Comfino.listItemContainer,
{characterData:!0,subtree:!0,childList:!0}));return}ComfinoPaywallFrontend.init(null,e,comfinoSettings.paywallOptions)}!Comfino.isSelected&&Array.isArray(a.paymentMethods)&&1===a.paymentMethods.length&&(ComfinoPaywallFrontend.logEvent("Firefox bug workaround executed.","debug",a.paymentMethods),Comfino.isSelected="comfino"===a.paymentMethods[0])}}};
