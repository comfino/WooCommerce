...
use ComfinoExternal\League\Flysystem\ConnectionErrorException;
use ComfinoExternal\League\Flysystem\ConnectionRuntimeException;
use ComfinoExternal\League\Flysystem\InvalidRootException;
...
class Ftp extends AbstractFtpAdapter
{
     ...
     * @var array
     */
    protected $configurable = ['host', 'port', 'username', 'password', 'ssl', 'timeout', 'root', 'permPrivate', 'permPublic', 'passive', 'transferMode', 'systemType', 'ignorePassiveAddress', 'recurseManually', 'utf8', 'enableTimestampsOnUnixListings'];
    /**
     * @var bool
     */
     ...
     * @param bool $passive
     */
    public function setPassive($passive = \true)
    {
    ...
    protected function login()
    {
        ...
        if (!$isLoggedIn) {
            $this->disconnect();
            throw new ConnectionRuntimeException('Could not login with connection: ' . $this->getHost() . '::' . $this->getPort() . ', username: ' . $this->getUsername());
        }
    }
    /**
    ...
    protected function createActualDirectory($directory, $connection)
    {
        ...
        if (in_array($directory, $listing, \true)) {
            return \true;
        }
    ...
    public function getMetadata($path)
    {
        ...
        if (@ftp_chdir($this->getConnection(), $path) === \true) {
            $this->setConnectionRoot();
            return ['type' => 'dir', 'path' => $path];
        }
        $listing = $this->ftpRawlist('-A', $path);
        if (empty($listing) || in_array('total 0', $listing, \true)) {
            return \false;
        }
